 ...
 *
 * vLLM exposes an OpenAI-compatible API, so we can reuse the OpenAI interaction class.
...
/**
 * vLLM uses the same request/response format as OpenAI.
 * This class handles vLLM chat completion interactions identically to OpenAI.
 */
class VllmChatCompletionInteraction implements InteractionUtils {
  // vLLM uses OpenAI-compatible types
  ...
  private mapToUiMessage(
    ...
    const vllmRoleToUIMessageRoleMap: Record<
    ...
    return {
      role: vllmRoleToUIMessageRoleMap[role],
      parts,
    };
  }
